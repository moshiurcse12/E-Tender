...
class PagesController extends Controller
{
  ...
  {
    $tenders = Tender::orderBy('id', 'DESC')->paginate(3);
  ...
  {
    $user = User::where('organization', $organization)->where('account_role', 'tenderer')->first();
    $categoryTenders = Tender::where('user_id', $user->id)
                       ->orderBy('id', 'DESC')->get();
  ...
  public function searchQuery(Request $request)
  {
    ...
    $categoryTenders = Tender::orWhere('title', 'like', '%'.$query.'%')
    ->orWhere('document_price', 'like', '%'.$query.'%')
    ->orWhere('security_amount', 'like', '%'.$query.'%')
    ->paginate(20);
