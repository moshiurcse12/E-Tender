        ...
        <ul class="list-group">
          @foreach(\App\Models\Tender::recentTenders() as $recentTender)
            <li class="list-group-item">
              <a style="color:#1b1c03;" href="{{ route('singleTender', $recentTender->slug) }}">{{ $recentTender->title }}</a>
            </li>
          @endforeach
        </ul>
      </marquee>
    ...
    <div>
      <ul class="list-group">
        @foreach (\App\Models\Tender::topTenders() as $topTender)
          <li class="list-group-item">
            <a style="color:#1b1c03;" href="{{ route('singleTender', $topTender->slug) }}">{{ $topTender->title }}</a>
          </li>
        @endforeach
      </ul>
    </div>
